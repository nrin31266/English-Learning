...
const getTier = (streak: number): StreakTier => {
  if (streak === 0) {
    return {
      title: "No streak yet",
      subtitle: "Complete a lesson today to start your streak.",
      nextMilestone: 1,
  ...
  if (streak < 3) {
    return {
      title: "First spark",
      subtitle: "Your learning habit has started.",
      nextMilestone: 3,
      container:
  ...
  if (streak < 15) {
    return {
      ...
      number: "text-white",
      accent:
        "ring-2 ring-orange-300/40 shadow-[0_0_12px_rgba(249,115,22,0.28)]",
  ...
  if (streak < 21) {
    return {
      ...
      number: "text-white",
      accent:
        "ring-2 ring-red-400/50 shadow-[0_0_14px_rgba(239,68,68,0.32)]",
      glow: "bg-red-300/35",
      progress: "bg-red-300",
    }
  }
  ...
  if (streak < 28) {
    return {
      ...
      number: "text-white",
      accent:
        "ring-2 ring-fuchsia-400/50 shadow-[0_0_16px_rgba(217,70,239,0.34)]",
  ...
  return {
    ...
    number: "text-white",
    accent:
      "ring-2 ring-cyan-300/60 shadow-[0_0_18px_rgba(34,211,238,0.38)]",
    glow: "bg-cyan-200/35",
...
const getMilestoneProgress = (streak: number, nextMilestone: number | null) => {
  ...
  const previous =
    [...previousMilestones].reverse().find((milestone) => streak >= milestone) ??
    0
...
const getStreakStatus = (
  ...
  if (!streakAlive) {
    return {
      title: "Streak lost",
      description: "Complete a lesson today to restart from 1 day.",
    }
  }

  if (canIncreaseStreakToday) {
    return {
      title: "Not secured today",
      description: `Complete a lesson today to keep your ${streak}-day streak.`,
    }
  }

  return {
    title: "Secured today",
    description: "Your streak is safe. Come back tomorrow to continue it.",
  }
}
...
const StreakBadge = ({
  ...
  const displayStreak = streakAlive ? streak : 0

  const tier = getTier(displayStreak)
  const inactiveTier = getTier(0)

  const shouldUseInactiveTrigger = !streakAlive || canIncreaseStreakToday
  const triggerTier = shouldUseInactiveTrigger ? inactiveTier : tier

  const displayValue = formatStreak(displayStreak)
  const isHot = displayStreak >= 4 && !shouldUseInactiveTrigger
  ...
  const daysToNext = tier.nextMilestone
    ? Math.max(0, tier.nextMilestone - displayStreak)
    : 0
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          aria-label={`Streak: ${displayStreak} days. ${streakStatus.title}`}
          className={cn(
            ...
            triggerTier.container,
            triggerTier.accent,
            className
          )}
        >
          {!shouldUseInactiveTrigger && displayStreak > 0 && (
            <span
              className={cn(
      ...
      <PopoverContent
        ...
        align="center"
        sideOffset={10}
        className="z-[100] w-[250px] overflow-hidden border-border/70 p-0 shadow-xl lg:w-[280px]"
      >
        <div className={cn("relative px-4 py-3 text-center", tier.container)}>
          ...
          <div className="relative flex flex-col items-center">
            <p className={cn("text-sm font-black lg:text-base", tier.number)}>
              {displayStreak.toLocaleString()} Day Streak
            </p>
        ...
        <div className="space-y-3 bg-popover px-4 py-3">
          <div className="rounded-xl bg-muted/50 px-3 py-2 text-center">
            <p className="text-xs font-semibold">{streakStatus.title}</p>
            <p className="mt-0.5 text-[11px] text-muted-foreground">
              {streakStatus.description}
            </p>
          </div>
